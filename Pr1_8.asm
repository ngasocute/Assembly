# ----------------------------------------------------------
 ...
 error_mssv : .asciiz " 20100000 <= MSSV < 30190000\ nMSSV khong trung nhau ."
 ...
 mark_and_name : .asciiz "\ nMSSV \t\ tDiem \ tHo va Ten \n"
 sorted_list : .asciiz "\ nDanh sach da sap xep :\n"
 so_sinh_vien : .word
 ten : .float 10.0
 zero : .float 0.0
 number : .space 4
 ...
 la $s5 , zero #$s5 = 0.0
 li $t2 , 101 #$t2 = 101
 li $t4 , 20100001 # Cho t4 = 20100001
 li $t5 , 30190000 # Cho t5 = 30190000
 jal __count # Ham nhap so luong SV nop
 la $s2 , number # s2 la dia chi bien number
 lw $s0 , 0( $s2 ) # Doc gia tri number cho vao s0
 move $s6 , $s0 # s6 , s7 = SSV
 move $s7 , $s0
 jal __input_info # Goi ham de nhap thong tin sinh vien nop
 li $v0 , 4 # In string
 la $a0 , st_list # In " Danh sach sinh vien "
 syscall
 jal __show_student # Goi ham show student nop
 jal __bubble_sort # Sap xep student theo diem tang dan nop
 li $v0 , 4 # In string
 la $a0 , sorted_list # In " Danh sach sinh vien da sap xep "
 syscall
 jal __show_student # In lai danh sach sau khi sap xep nop
 ...
 j do_count
 __input_info :
 li $t0 , 0 # khoi tao $t0 = 0 , $t0[U+FFFD] l bien dem (i) cua sinh vien vua duoc nhap thong tin
 ...
 slt $v0 , $a0 , $t4 # Neu a0 (so sinh vien ) < t4 ( = 20100001 )--> v0 = 1
 bnez $v0 , re_input_mssv # v0 != 0 --> Nhap lai
 slt $v0 , $a0 , $t5 # Neu a0 (so sinh vien ) < t5 ( = 30190000 )--> v0 = 1
 ...
 la $a1 , 8( $t1 ) # Chi ra vi tr[U+FFFD] luu ten
 ...
 # Thoat nhap diem
 exit_do :
s.s $f0 , 4( $t1 ) # Luu diem[U+FFFD] v omang
...
li $t3 , 0 #[U+FFFD] g nj = 0 ( t3 la bien dem 2 tu trai sang j)
...
la $t1 , students # Nap dia chi dau mang cac block luu thongtin sinh vien
la $a0 , mark_and_name # in ra chuoi " Diem Ho va Ten "
syscall
li $t0 , 0 # Khoi tao $t0 = 0 , $t0[U+FFFD] l bien diem so sinh vien da duoc duyet
loop3 :
slt $v0 , $t0 , $s6 # So sanh $t0 (So sv da duyet )[U+FFFD] v $s0 (Tong so sinh vien )
beqz $v0 , exit_show # Thoat vong lap khi duyet het sinh vien
li $v0 , 1 # Hien thi console
lw $a0 , 0( $t1 ) # In mssv ra[U+FFFD] m nhinh console
syscall
li $v0 , 11 # In char
li $a0 , '\t'  # In dau tab
syscall
