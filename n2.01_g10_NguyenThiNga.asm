#Problem 2.1: Marbot
#Author: Nguyen Thi Nga - 20184296
...
#-------------------------------------------------------------------------------
	...
	nowHeading: 		.word 0
#---------------------------------------------------------
# the path of marbot is stored in array path
# each edge is stored in a type of structure {x, y, z}
# where: 	x, y is the first co-ordinate
#		z is direction
# default:	first structure is {0,0,0}
# the length of path when begin is 12 bytes (3x 4byte)
#---------------------------------------------------------
	path: 		.space 600
...
CheckControlCode:
		...
		lw 	$s2, 0($s2)
		#----------------
		bne 	$s2, 3, pushErrorMess 	# check length code 
...
GO: 	#backup
	...
	sw 	$k0,0($sp)
	#processing
	li 	$at, MOVING 	# change MOVING port, mã moving có s?n, m?ng có 1 ph?n t?
...
#--------------------------------------------------------
get_cod:
	li 	$t1, IN_ADRESS_HEXA_KEYBOARD # nhan biet xem dang dùng o hàng nào
	li 	$t2, OUT_ADRESS_HEXA_KEYBOARD # nhan biet dang an phím nào
scan_row1:
	li 	$t3, 0x81	# load vào trong out_adress
	sb 	$t3, 0($t1)
	lbu 	$a0, 0($t2)
	bnez 	$a0, get_code_in_char 	# có 1 phím ?c ?n
scan_row2:
	li 	$t3, 0x82
	sb 	$t3, 0($t1)
	lbu 	$a0, 0($t2)
...
store_code:
# array: inputControlCode và lengthControlCode
	la 	$s1, inputControlCode
	la 	$s2, lengthControlCode
	
	lw 	$s3, 0($s2)				#$s3 = strlen(inputControlCode)
	addi 	$t4, $t4, -1 	#counter			#$t4 = i 
	for_loop_to_store_code:
		addi 	$t4, $t4, 1 # i++
		bne 	$t4, $s3, for_loop_to_store_code # chi dùng khi array dã có phan tu roi, ?? cho counter ch?y 
		
		add 	$s1, $s1, $t4		#$s1 = inputControlCode + i truy c?p ??n v? trí nào ??y
